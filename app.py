...
def initialize_persistent_report_state():
    ...
    if "ar_income_data" not in st.session_state:
        saved = load_persistent_report("ar_income_report")
        if saved:
            st.session_state["ar_income_data"] = saved
...
def format_saved_timestamp(iso_text):
    """Display a saved timestamp in a clean manager-facing format."""
    if not iso_text:
...
def save_ar_income(amount):
    payload = {
        "weekly_income": float(amount or 0),
        "saved_at": datetime.now().isoformat(),
        "source_label": "Manual Entry",
    }
    st.session_state["ar_income_data"] = payload
    save_persistent_report("ar_income_report", payload)
    return payload
...
def apply_diamond_theme():
    st.markdown(
        ...
        :root {
            --diamond-red: #0B5CAD;
            --diamond-red-dark: #084A8A;
            --diamond-black: #0B2A4A;
            --diamond-charcoal: #123B63;
            --diamond-gray: #F5FAFF;
            --diamond-border: #C7DFF5;
        }
        ...
        .diamond-header {
            background: linear-gradient(135deg, #ffffff 0%, #F7FBFF 60%, #EAF4FF 100%);
        ...
        div[data-testid="stMetric"] {
            ...
            border-radius: 14px;
            padding: 14px 16px;
            box-shadow: 0 4px 14px rgba(11,92,173,.08);
        }
        div[data-testid="stMetric"] *,
        ...
        [data-testid="stFileUploader"] section {
            ...
            border: 2px dashed #8ABBE8 !important;
...
st.markdown(
    ...
    <style>
    .home-hero {
        background: linear-gradient(135deg, #0B2A4A 0%, #0B5CAD 60%, #1E88E5 100%);
        border-left: 10px solid #0B5CAD;
        border-radius: 22px;
        padding: 34px 34px 30px 34px;
        margin: 8px 0 22px 0;
        box-shadow: 0 14px 34px rgba(11,92,173,.22);
    }
    .home-hero img {
        max-width: 380px;
        width: 100%;
    ...
    .home-note {
        ...
        border: 1px solid rgba(11,92,173,.38);
        border-radius: 12px;
        padding: 12px 14px;
        margin-top: 18px;
    ...
    .home-card {
        ...
        min-height: 178px;
        box-shadow: 0 6px 18px rgba(11,92,173,.08);
    }
    .home-card-title {
        font-size: 1.08rem;
        font-weight: 900;
    ...
    .summary-card {
        ...
        border-left: 6px solid #0B5CAD !important;
        border-radius: 14px !important;
    ...
    st.markdown(
        ...
        <div class="home-hero">
            ...
            <div class="home-subtitle">
                One central place for managers to review weekly cash flow, A/R income, A/P aging detail, and P&L reports.
            </div>
    ...
    with c2:
        st.markdown(
            ...
            <div class="home-card">
                ...
                <div class="home-card-text">
                    Manually enter the weekly income number and keep the latest amount saved until it is updated.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
...
def render_ar_page():
    render_brand_header()
    st.subheader("A/R")
    st.caption("Enter the weekly income number for White Line Trucking. The latest saved number stays visible until it is updated.")

    ar_income_data = st.session_state.get("ar_income_data")
    current_amount = money_to_float(ar_income_data.get("weekly_income", 0)) if ar_income_data else 0.0
    saved_at = ar_income_data.get("saved_at", "") if ar_income_data else ""
    source_label = ar_income_data.get("source_label", "Manual Entry") if ar_income_data else "Manual Entry"

    render_summary_cards([
        ("Weekly Income", f"${current_amount:,.2f}" if ar_income_data else "—", "latest saved number" if ar_income_data else "waiting for entry"),
        ("Source", source_label, "manual entry"),
        ("Last Updated", format_saved_timestamp(saved_at), "stays until updated" if ar_income_data else "not saved yet"),
    ])

    if ar_income_data:
        st.success("Showing the latest saved A/R weekly income number.")
        st.dataframe(
            [{
                "Weekly Income": f"${current_amount:,.2f}",
                "Last Updated": format_saved_timestamp(saved_at),
            }],
            use_container_width=True,
            hide_index=True,
        )
        edit_mode = st.checkbox("Update / edit this weekly income number", key="ar_income_edit_mode")
    else:
        st.info("No A/R weekly income number has been saved yet. Enter the amount below and click Save.")
        edit_mode = True

    if edit_mode:
        with st.container(border=True):
            st.markdown("**A/R Weekly Income Entry**")
            income_value = st.number_input(
                ...
                step=100.0,
                format="%.2f",
                key="ar_income_entry_amount",
            )
            if st.button("Save weekly income", key="save_ar_income_button", type="primary"):
                save_ar_income(income_value)
                st.success("A/R weekly income saved successfully.")
                st.rerun()

    st.caption("This page is manual-entry only for White Line Trucking. It does not require a PDF or QuickBooks upload.")
